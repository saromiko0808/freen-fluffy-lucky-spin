(() => {
  ...
  let save=readSave(),levelIndex=0,score=0,spins=0,busy=false,noWin=0,attempt=0,sound=save.sound!==false,currentSymbols=[],holdArmed=false;
  ...
  function renderRunningReels(){const cfg=levels[levelIndex];$('#reels').innerHTML=Array.from({length:3},(_,reelIndex)=>{const strip=Array.from({length:4},()=>makeFreen(cfg));strip.push({...strip[0]});return `<div class="reel live" style="--reel-speed:${[.76,.62,.7][reelIndex]}s"><div class="reel-strip">${strip.map(symbolHTML).join('')}</div></div>`}).join('')}
  ...
  function beginHold(event){if(busy||spins<=0)return;event.preventDefault();holdArmed=true;if(event.pointerId!==undefined)$('#spinBtn').setPointerCapture?.(event.pointerId);$('#spinBtn').classList.add('charging','armed');$('#spinHelp').textContent='轉動中・放開停止';$('#message').textContent='Freen、Fluffy，連結愛與幸運…';renderRunningReels()}
  function endHold(event){event.preventDefault();if(!holdArmed)return;holdArmed=false;$('#spinBtn').classList.remove('charging','armed');$('#spinHelp').textContent='按住轉動・隨時放開';if(navigator.vibrate)navigator.vibrate(18);spin()}
  function cancelHold(){if(!holdArmed)return;holdArmed=false;$('#spinBtn').classList.remove('charging','armed');$('#spinHelp').textContent='按住轉動・隨時放開';renderReels(currentSymbols)}
  $('#startBtn').onclick=()=>{renderLevels();show('levels')};$('#continueBtn').onclick=()=>startLevel(Math.min(save.unlocked-1,11));$$('[data-home]').forEach(b=>b.onclick=()=>show('home'));$('#backLevels').onclick=()=>{renderLevels();show('levels')};$('#spinBtn').addEventListener('pointerdown',beginHold);$('#spinBtn').addEventListener('pointerup',endHold);$('#spinBtn').addEventListener('pointercancel',cancelHold);$('#spinBtn').oncontextmenu=e=>e.preventDefault();$('#nextBtn').onclick=()=>startLevel(levelIndex+1);$('#retryBtn').onclick=()=>startLevel(levelIndex);$('#chooseBtn').onclick=()=>{renderLevels();show('levels')};$('#collectionBtn').onclick=renderCollection;$('#collectionBack').onclick=()=>{renderLevels();show('levels')};$$('.sound-btn').forEach(b=>b.onclick=()=>{sound=!sound;writeSave();$$('.sound-btn').forEach(x=>x.textContent=sound?'♪':'×');toast(sound?'聲音已開啟':'聲音已關閉')});renderLevels();$$('.sound-btn').forEach(x=>x.textContent=sound?'♪':'×')
})();
